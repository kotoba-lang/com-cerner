(ns cerner.main
  "Kotodama WASM entrypoint for the Cerner clean-room actor (L5) — Clojure port.
  ...
  facts (`cerner.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cerner")
(def tier "L5")
...
(def entity-specs
  [{:entity "Patient"              :plural "patients"              :id-prefix "cerner_pat"
    :fields [:active :gender :birthDate :deceasedBoolean :deceasedDateTime :multipleBirthBoolean :multipleBirthInteger] :required [:active :gender]
    :coerce {:active :bool :deceasedBoolean :bool :multipleBirthBoolean :bool :multipleBirthInteger :int} :refs {}}
   {:entity "Observation"          :plural "observations"          :id-prefix "cerner_obs"
    :fields [:status :effectiveDateTime :effectiveInstant :issued :valueString :valueBoolean :valueInteger :valueDateTime] :required [:status :effectiveDateTime]
    :coerce {:valueBoolean :bool :valueInteger :int} :refs {}}
   {:entity "Encounter"            :plural "encounters"            :id-prefix "cerner_enc"
    :fields [:status]              :required [:status]
    :coerce {}                       :refs {}}
   {:entity "Condition"            :plural "conditions"            :id-prefix "cerner_con"
    :fields [:recordedDate :onsetDateTime :abatementDateTime] :required [:recordedDate :onsetDateTime]
    :coerce {}                       :refs {}}
   {:entity "MedicationRequest"   :plural "medicationrequests"   :id-prefix "cerner_med"
    :fields [:status :intent :priority :doNotPerform :reportedBoolean :authoredOn] :required [:status :intent]
    :coerce {:doNotPerform :bool :reportedBoolean :bool} :refs {}}
   {:entity "AllergyIntolerance"  :plural "allergyintolerances"  :id-prefix "cerner_all"
    :fields [:type :criticality :recordedDate :onsetDateTime :lastOccurrence] :required [:type :criticality]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"cerner.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cerner-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cerner-compat/ready)
